...
import profileImg from '../assets/profile-square-640.jpg';

export const Home: React.FC = () => {
  return (
    <div className="animate-fade-in max-w-3xl">
      <div className="space-y-12">
        {/* Hero section: centered photo above name and title */}
        <div className="flex flex-col items-center text-center space-y-6">
          <div className="w-36 h-36 md:w-44 md:h-44 rounded-full overflow-hidden border-2 border-brand-border shadow-md">
            <img
              src={profileImg}
          ...
          <div className="space-y-4">
            ...
            <h1 className="text-3xl md:text-4xl font-bold leading-tight tracking-tight text-brand-dark font-serif">
              {PERSONAL_INFO.name}
            </h1>
            <p className="text-base md:text-lg text-brand-muted italic font-serif leading-relaxed tracking-tight">
              {PERSONAL_INFO.title}
            </p>
          </div>
        </div>

        <div className="text-brand-text leading-[1.8] max-w-none space-y-5 text-base md:text-[1.05rem]">
          <p>
        ...
        {NEWS_ITEMS.length > 0 && (
          <section className="pt-10 border-t border-brand-border">
            ...
            <div className="space-y-6">
              {NEWS_ITEMS.map((item) => (
                <div key={item.id} className="group bg-brand-card border border-brand-border hover:border-brand-muted/30 rounded-sm overflow-hidden transition-all duration-300">
                  {item.image && (
                    <div className="aspect-[21/9] overflow-hidden border-b border-brand-border">
                      <img
                        src={item.image}
                        alt={item.title}
                        loading="lazy"
